...
def get_data():
    ...
    label2id = load_label2id('../../thumos_annotations/Class Index_Detection.txt')

    for vid in to_visualize_name:
        draw(results[vid], to_visualize_name[vid], f'{video_dir}/{vid}.mp4',
             f'{output_dir}/{vid}.jpg', label2id)
        print(f'{vid}.jpg generated!')


def draw(result, info, video_path, output_file, label2id):
    # 绘图
    total_num = 1000
    end = 100
    ...
    for i in range(len(result)):
        onset, offset = result[i]['segment']
        score = result[i]['score']
        label = label2id[result[i]['label']]
